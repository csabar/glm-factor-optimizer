...
def make_count_rate_data(n_rows: int = 2_000, seed: int = 42) -> pd.DataFrame:
    ...
    segment = rng.choice(["basic", "standard", "premium"], p=[0.35, 0.45, 0.20], size=n_rows)
    region = rng.choice(["north", "south", "east", "west"], size=n_rows)
    segment_effect = pd.Series(segment).map({"basic": 0.20, "standard": 0.0, "premium": -0.25}).to_numpy()
...
def make_gamma_severity_data(n_rows: int = 2_000, seed: int = 43) -> pd.DataFrame:
    ...
    score = rng.normal(size=n_rows)
    segment = rng.choice(["basic", "standard", "premium"], p=[0.35, 0.45, 0.20], size=n_rows)
    segment_effect = pd.Series(segment).map({"basic": -0.10, "standard": 0.0, "premium": 0.30}).to_numpy()
